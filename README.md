...
This is a web app built using React and is used to visualize classic sorting algorithms such as insertion sort, merge sort, quick sort, etc.
...
- `components` - This folder is broken down into atoms, molecules and organisms subfolders
  ...
  - Each component is contained in its own folder and has 2 files. The `index.js` file contains the JavaScript code for the component and the `style.css` file contains rules for classes which are written using an alternate style naming scheme  that is described as follows:
